            ...
            switch (_c.label) {
                case 0:
                    ...
                    return [4 /*yield*/, Promise.all(users.map(function (data) { return __awaiter(_this, void 0, void 0, function () {
                            var email, password, data_1, field, fieldType, value;
                            return __generator(this, function (_a) {
                                email = data.email;
                                password = data.password;
                                try {
                                    firebase_1.auth.createUserWithEmailAndPassword(email, password);
                                }
                                catch (error) {
                                ...
                                if (userFields) {
                                    ...
                                    for (field in userFields) {
                                        ...
                                        if (userFields[field]) {
                                            ...
                                            data_1[field] = value; // overwrite teh previus value with the new random value
                                        }
                                    }
                                    console.log(typeof firebase_1.fs.collection('users'));
                                    if (usersCollectionReference &&
                                        typeof usersCollectionReference === typeof firebase_1.fs.collection('users')) {
                                        usersCollectionReference.add(__assign({ password: password, email: email }, data_1));
                                    }
                                    else {
                                        firebase_1.fs.collection('users').add(__assign({ password: password, email: email }, data_1));
                                    }
                                }
                                return [2 /*return*/];
                            });
